...
=== Introduction
The OCloud framework helps operators to build enterprise cloud services on Oracle Cloud Infrastructure (OCI). While launching a server in a public cloud has become a matter of pressing a button, building a hosting platform for intra- and extranet services is more challenging. This framework aims at system administrators building enterprise services. link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] is employed to address advanced compliance and security requirements with agility, utilizing automation scipts that provision dedicated infrastructure and deploy operator controls together with a service. The framework helps operators to launch a new service while they still learn to utilize the numerous configuration options. It provides a baseline configuration through parameters that are separated from the actual deployment code and avoids to prescribe a bespoke topology. Relying on state-aware provisioning through the link:https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Concepts/resourcemanager.htm[Oracle's Resource Manager (ORM)] operators adjust the topology when requirements evolve. For one-time deployments, the link:https://cloud.oracle.com/resourcemanager/stacks/create?zipUrl=https://github.com/oracle-devrel/terraform-oci-ocloud-landing-zone/archive/refs/heads/main.zip[Deploy to the Oracle Cloud] button creates a zip archive that is pushed to the resource manager directly, to enable continuous changes the code should be cloned into a private repository and be connected as a source provider.
