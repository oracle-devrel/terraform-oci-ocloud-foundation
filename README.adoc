...
The script is meant for a initial tenancy setup and provisions global resources like compartments and groups. The naming should be unique though. While compartment names are constructed to avoid conflicts, groups are resources and name conflicts can be avoided to check existing groups against the link:doc/naming.adoc[ocloud default names].
