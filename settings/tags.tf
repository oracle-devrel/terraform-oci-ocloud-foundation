...
locals {
    ...
    defaultvalues    = merge([for collection, tags in var.collections: { for tag, values in tags: tag => flatten([values])[0] }]...)
    default_tags = flatten([ for collection, tags in var.collections : 
        [ for tag, values in tags : {
            namespace    = collection
            namespace_id = "open"
            name         = tag
            values       = flatten([values])
            type         = length(flatten([values])) != 1 ? "ENUM" : "DEFAULT"
            cost_tracker = collection == "budget" ? true : false
        }]
    ])
...
output "default_values"  { value = local.defaultvalues }