...
| <a name="input_bundles"></a> [bundles](#input\_bundles) | --- config --- The bundle argument allows to define provisioning tiers with "count = module.compose.bundle\_id >= 2 ? 1 : 0" | `map(number)` | <pre>{<br>  "free_tier": 1,<br>  "payg": 2,<br>  "premium": 4,<br>  "standard": 3<br>}</pre> | no |
