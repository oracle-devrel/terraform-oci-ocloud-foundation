...
data "oci_identity_compartment" "service" { id        = var.service.service_id }
data "oci_core_subnet"          "domain"  { subnet_id = var.service.subnet_ids[0] }

// --- Get all the Availability Domains for the region
data "oci_identity_availability_domains" "host" { compartment_id = var.service.compartment_id }
...
data "oci_core_shapes" "ad1" {
  compartment_id      = var.service.compartment_id
  availability_domain = local.ADs[0]
}

data "oci_core_subnet" "host" {
  count     = length(var.service.subnet_ids)
  subnet_id = element(var.service.subnet_ids, count.index)
}

// --- Bastion Datasource  ----
data "oci_bastion_bastions" "host" {
  compartment_id          = data.oci_core_subnet.segment.compartment_id
  bastion_id              = var.service.bastion_id
...
data "oci_core_instance_credentials" "host" {
  ...
  count       = module.host_settings.image.resource_platform != "linux" ? module.host_settings.shape.count : 0
  ...
  part {
    ...
    content = templatefile(
      local.cloudinit, {
        ...
        timezone     = module.host_settings.image.timezone,
      }
    )
  }
}
...
locals {
  ...
  default_freeform_tags = {
    # ocloud list of freeform tags are added by default to user provided freeform tags (var.service.freeform_tags) if local.merged_freeform_tags is used
    terraformed = "Please do not edit manually"
    module      = "oracle-terraform-modules/compute-instance/oci"
  }
  merged_freeform_tags = merge(local.default_freeform_tags, var.service.freeform_tags)
  shapes_config = {
  ...
  shape_is_flex = length(regexall("^*.Flex", module.host_settings.shape.shape)) > 0 # evaluates to boolean true when var.instance.shape contains .Flex
  instances_details = [
