...
locals {
    ...
    nat_route = {
        network_entity_id = oci_core_nat_gateway.ocloud.id
        description       = "Route traffic via NAT to the public internet"
        destination       = var.vcn.address_spaces.anywhere
        destination_type  = "CIDR_BLOCK"
    }
    anywhere_route = {
        network_entity_id = oci_core_internet_gateway.ocloud.id
        description       = "Route traffic to the public internet"
        destination       = var.vcn.address_spaces.anywhere
        destination_type  = "CIDR_BLOCK"
    }
    objectstorage_route  = {
        network_entity_id = oci_core_service_gateway.ocloud.id
    ...
    interconnect = {
        network_entity_id = oci_core_drg.ocloud[0].id
